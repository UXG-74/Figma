  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center px-8 text-center py-20">
      <div className="max-w-md mx-auto">
        <div className="w-16 h-16 rounded-full bg-[var(--c-bg-soft)] border border-[#e5c3c3] flex items-center justify-center mx-auto mb-8">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#c9a0a0" strokeWidth="1.5">
            <path d="M20 6 9 17l-5-5"/>
          </svg>
        </div>

        <p className="text-label text-[9px] text-[var(--c-text-muted)] mb-3">Thank you</p>
        <h1 className="font-display text-4xl md:text-5xl font-medium mb-4">Your order is confirmed</h1>
        <p className="text-body-sm text-[var(--c-text-muted)] mb-2">A confirmation has been sent to your email address.</p>
        <p className="text-body-sm text-[var(--c-text-muted)] mb-8">
          Order reference: <span className="font-medium text-[var(--c-text)]">{orderNumber}</span>
        </p>

        <div className="bg-[var(--c-bg-soft)] border border-[var(--c-border)] p-6 text-left mb-8 space-y-4">
          {[
            ...
            <div key={k} className="flex justify-between">
              <span className="text-body-sm text-[var(--c-text-muted)]">{k}</span>
              <span className="text-body-sm font-medium">{v}</span>
            </div>
          ))}
        </div>

        <p className="font-display text-lg italic text-[var(--c-text-muted)] mb-10">
