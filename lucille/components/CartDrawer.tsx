...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 flex flex-col animate-slide-in shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-6 border-b border-[#e8e0dc]">
          <div>
            <h2 className="font-display text-xl">Your Bag</h2>
            <p className="text-body-sm text-[#8c7b7b] mt-0.5">{items.length} {items.length === 1 ? 'item' : 'items'}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              <div className="w-16 h-16 rounded-full bg-[#faf7f5] flex items-center justify-center">
              ...
              <p className="text-body-sm text-[#8c7b7b]">Discover something beautiful</p>
              ...
              {items.map(item => (
                <div key={`${item.product.id}-${item.size}-${item.colour}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-body-sm text-[#8c7b7b] mt-1">{item.colour} · {item.size}</p>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <button onClick={() => updateQuantity(item.product.id, item.size, item.colour, item.quantity - 1)}
                        className="w-6 h-6 border border-[#e8e0dc] flex items-center justify-center text-sm hover:border-[#1a1a1a] transition-colors">−</button>
                      ...
                      <button onClick={() => removeItem(item.product.id, item.size, item.colour)}
                        className="ml-auto text-[#8c7b7b] hover:text-[#1a1a1a] transition-colors">
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="px-8 py-6 border-t border-[#e8e0dc] space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-body-sm text-[#8c7b7b]">Subtotal</span>
              <span className="font-display text-lg">£{total().toFixed(2)}</span>
            </div>
            <p className="text-body-sm text-[#8c7b7b] text-center">Shipping & taxes calculated at checkout</p>
